...
MIN_ALTITUDE_MEO_SATELLITE_STATION_IN_KMS = 2000
"""
The minimum altitude in KMs (Kilometers) for a
Qiskrypt's MEO (Medium-Earth Orbit) Satellite Station.
"""

MAX_ALTITUDE_MEO_SATELLITE_STATION_IN_KMS = 35786
"""
...
class QiskryptMEOSatelliteStationEndpoint(QiskryptSatelliteStationEndpoint):
    """
    Object class for the Qiskrypt's MEO (Medium-Earth Orbit) Satellite Station Endpoint.
    """

    def __init__(self, num: int, name: str, context: str,
                 longitude: str, latitude: str, altitude_in_kms: str):
        """
        Constructor of the Qiskrypt's MEO (Medium-Earth Orbit) Satellite Station Endpoint.
        ...
        if MIN_ALTITUDE_MEO_SATELLITE_STATION_IN_KMS <= \
            float(altitude_in_kms) <= \
                MAX_ALTITUDE_MEO_SATELLITE_STATION_IN_KMS:
            """
            If the altitude of the Qiskrypt's HEO (High-Earth Orbit) Satellite Station Endpoint is valid,
            i.e., between 2,000 KMs (Kilometers) and 35,786 KMs (Kilometers).
            """

            super().__init__(num, name, context,
                             longitude, latitude, altitude_in_kms,
                             POSSIBLE_SATELLITE_STATION_ORBIT_TYPES[1])
            """
            ...
            i.e., lower than 2,000 KMs (Kilometers) or higher than 35,786 KMs (Kilometers).
            """
