...
MAX_ALTITUDE_GROUND_STATION_IN_KMS = 8
"""
The maximum altitude in KMs (Kilometers) for a Qiskrypt's Ground Station.

NOTE:
- Based on the highest place known on Earth.
- Mount Everest (China/Nepal) for reference.
"""


class QiskryptGroundStationEndpoint(QiskryptEndpoint):
    """
    Object class for the Qiskrypt's Ground Station Endpoint.
    """

    def __init__(self, num: int, name: str, context: str,
                 longitude: str, latitude: str, altitude_in_kms: str):
        """
        Constructor of the Qiskrypt's Ground Station Endpoint.
        ...
        if float(altitude_in_kms) <= MAX_ALTITUDE_GROUND_STATION_IN_KMS:
            """
            If the altitude of the Qiskrypt's Ground Station Endpoint is valid,
            i.e., lower than or equal to 8 KMs (Kilometers).
            """
            ...
            """
            If the altitude of the Qiskrypt's Ground Station Endpoint is not valid,
            i.e., higher than 8 KMs (Kilometers).
            """
